...
public class ChargeTime {
...

		{
			...
			if (multi) {
				...
				while (thisDay <= numberOfDays) {
					...
					String lastMin = LAST_MIN_OF_DAY;
					long minutes;

					// get first day
					if (thisDay == 1) {
						minutes = numberOfMinutes(this.start.substring(TIME_START), lastMin) + 1;
					} else if (thisDay == numberOfDays) {
						minutes = numberOfMinutes(MIDNIGHT, this.stop.substring(TIME_START));
					} else {
						minutes = numberOfMinutes(MIDNIGHT, lastMin) + 1;
					}
					timeTotal += ("Day " + thisDay + " minutes: " + minutes + "\n");
					thisDay++;
				}
			}
	...
	private boolean isNewYear() {
		if (start.substring(MONTH_START, MONTH_END).equals(DEC) && stop.substring(MONTH_START, MONTH_END).equals(JAN)) {
			return true;
		}
		return false;
	}
